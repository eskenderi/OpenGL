...
public:
	...
	bool barClicked(int x_, int y_) {//Here we manage the movement of the slider by clicking on its bar
		int margin = 5;//set a margin click around the x coordinate of the bar to make it more easy to click
...
void mouse(int button, int state, int x, int y) {
	if (button == GLUT_LEFT_BUTTON) {
		if (x < 400) {//Slider's region...
			switch (state)
			{
			case (GLUT_DOWN)://On mouse left click down check whether mouse is over either one of the slider's button
...
void motion(int mouseX, int mouseY) {
	if (mouseX < 400) {// Slider's side
		if (rubberbanding) {//If any slider is being dragged, keep changing it's value(position) continuously
